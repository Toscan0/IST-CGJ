...
// Method to load an image into a texture using the freeimage library. Returns the texture ID or dies trying.
GLuint image::loadTexture(string filenameString) // , GLenum minificationFilter = GL_LINEAR, GLenum magnificationFilter = GL_LINEAR)
{
...
void image::snapShot(int width, int height)
{
	...
	// Martelada para trocar o lugar do Red pelo blue porque as masks nao estao a funcionar 
	GLubyte* pixelsRGB = new GLubyte[3 * width * height];
	for (int i = 0; i < width * height; i++) {
		pixelsRGB[(i * 3)] = pixels[i * 3 + 2];
		pixelsRGB[(i * 3) + 1] = pixels[i * 3 + 1];
		pixelsRGB[(i * 3) + 2] = pixels[i * 3];
	}

	// Convert to FreeImage format & Save
	FIBITMAP* image = FreeImage_ConvertFromRawBits(pixelsRGB, width, height, 3 * width, 24, 0x0000FF, 0x00FF00, 0xFF0000, false);

	snapCounter++;
	std::string path1 = "../../assets/images/SnapShot";
	std::string path2 = ".jpg";
	std::string number = std::to_string(snapCounter);
	std::string path = path1 + number + path2;
